...
public:
    ...
    // SymGetModuleInfo64()
    typedef BOOL (__stdcall *tSGMI)( IN HANDLE hProcess, IN DWORD64 dwAddr, OUT IMAGEHLP_MODULE64_V3 *bModuleInfo );
    tSGMI pSGMI;
...
BOOL StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
    ...
    // two passes:
    // pass 1: compute the space reqd.
    // pass 2: actually copy in the stackframes
    totalSize= 0;
    for (int pass = 0; pass < 2; pass++)
    {
        // init STACKFRAME and context for first call
        STACKFRAME64 s; // in/out stackframe
        memset(&s, 0, sizeof(s));
        DWORD imageType;
        CHAR stackFrameBuffer[STACKWALK_MAX_NAMELEN];
...
#error "Platform not supported!"
        ...
        for (frameNum = 0; ; ++frameNum )
        {
            ...
            this->OnCallstackEntry(et, csEntry, stackFrameBuffer, STACKWALK_MAX_NAMELEN);
            if (pass==0)
                totalSize += strlen(stackFrameBuffer)+2;
            else
                strcat_s(stackTraceBuffer, totalSize, stackFrameBuffer);

          
            if (s.AddrReturn.Offset == 0)
            {
                this->OnCallstackEntry(lastEntry, csEntry, stackFrameBuffer, STACKWALK_MAX_NAMELEN);
                if (pass==0)
                    totalSize += strlen(stackFrameBuffer)+2;
                else
                    strcat_s(stackTraceBuffer, totalSize, stackFrameBuffer);
        ...
        if (pass==0)
        {
            totalSize += 2 + strlen ("Stack trace:   ");
            stackTraceBuffer = (CHAR *) malloc (totalSize);
            if (!stackTraceBuffer) goto cleanup;
          
            strcpy_s(stackTraceBuffer, totalSize, "Stack trace:\n");
        }
      
    } // for (2 passes)

    // emit the entire stack trace in one go
    OnOutput(stackTraceBuffer);
  
  cleanup:
    if (pSym) free( pSym );
    if (stackTraceBuffer) free( stackTraceBuffer );
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, CHAR* buffer, size_t len)
{
    //CHAR buffer[STACKWALK_MAX_NAMELEN];
    if ( (eType != lastEntry) && (entry.offset != 0) )
    {
        ...
        if (entry.lineFileName[0] == 0)
        {
            ...
            _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, len, "%p module(%s): %s: %s\n", (LPVOID) entry.offset, entry.moduleName, entry.lineFileName, entry.name);
        }
        else
            _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, len, "%s (%d): %s\n", entry.lineFileName, entry.lineNumber, entry.name);
        //OnOutput(buffer);
    }
}
...
void StackWalker::OnDbgHelpErr(LPCSTR szFuncName, DWORD gle, DWORD64 addr)
{
    ...
    if (_strnicmp(szFuncName, "SymGetLineFromAddr64", strlen("SymGetLineFromAddr64"))!=0
        && _strnicmp(szFuncName, "SymGetSymFromAddr64", strlen("SymGetSymFromAddr64"))!=0)
    {
